...
class IkSaberD6x:
    ...
    def __init__(self):
        ### utility
        self.__data_interface = DataInterface("ik_saber_d6x")
        ...
        self.__cur_jnt_state = HexBaseJntState(
            position=np.zeros(self.__dyn_util.get_joint_num()),
            velocity=np.zeros(self.__dyn_util.get_joint_num()),
            effort=np.zeros(self.__dyn_util.get_joint_num()),
        )
    ...
    def run(self):
        while self.__data_interface.ok():
            # update target pose
            cur_tar_pose = None
            for cur_tar_pose in iter(self.__data_interface.get_target_pose()):
                pass
            ...
            if self.__cur_tar_se3 is not None:
                ...
                if success:
                    self.__cur_jnt_state.position = tar_jnt_state.position
                    ...
                    if self.__prog_param["debug"]:
                        tar_ee_pos, tar_ee_quat = self.__dyn_util.forward_kinematics(
                            self.__cur_jnt_state.position)[-1]
